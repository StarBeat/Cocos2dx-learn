...
LightEffect* LightEffect::create(const std::string& fs, const std::string& vs)
{
	auto eff = new LightEffect();
	if (eff && eff->init(fs, vs))
	{
		eff->autorelease();
		return eff;
	}
...
bool LightEffect::init(const std::string& fs, const std::string& vs)
{
	if (initGLProgramState(fs, vs))
	{
...
void LightEffect::prepareForRender(cocos2d::Sprite* sprite, cocos2d::Texture2D* texture)
{
	...
	gl->setUniformTexture("u_normals", texture);
